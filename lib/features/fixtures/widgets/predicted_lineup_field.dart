...
class PredictedLineupField extends StatelessWidget {
          ...
          children: [
            ...
            SizedBox(
              ...
              child: Column(
                children: [
                  // Away team (top half)
                  Expanded(child: _buildTeamHalf(awayLineup, isHome: false)),

                  // Divider with team names and formations
                  _buildCenterLine(context),

                  // Home team (bottom half)
                  Expanded(child: _buildTeamHalf(homeLineup, isHome: true)),
                ],
              ),
            ),
          ],
  ...
  Widget _buildTeamHalf(PredictedLineup? lineup, {required bool isHome}) {
    ...
    for (final player in lineup.starters) {
      ...
      final line =
          ...
          _inferLineFromPosition(player.position, totalLines);
    ...
    return Padding(
      ...
      child: Column(
        children: orderedLines.map((line) {
          ...
          return Expanded(child: _buildPositionRow(players));
        }).toList(),
      ),
    );
  }
  ...
  int _inferLineFromPosition(String position, int totalLines) {
    final lastOutfieldLine = totalLines;
    final firstMidfieldLine = totalLines > 3 ? 3 : 2;
    final lastMidfieldLine = totalLines > 4
        ? totalLines - 1
        : firstMidfieldLine;
    ...
    switch (position.toUpperCase()) {
      ...
      case 'DEF':
        return 2;
      case 'MID':
        return ((firstMidfieldLine + lastMidfieldLine) / 2).floor();
      case 'FWD':
        return lastOutfieldLine;
      default:
        return ((firstMidfieldLine + lastMidfieldLine) / 2).floor();
    }
  }
              ...
              if (player.isReturningFromInjury ||
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    height: 16,
                    decoration: BoxDecoration(
                      color: player.isReturningFromInjury
                          ? Colors.green
                          : Colors.blue,
                      shape: BoxShape.circle,
