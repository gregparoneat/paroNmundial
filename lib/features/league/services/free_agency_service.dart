...
class FreeAgencyService extends ChangeNotifier {
  ...
  List<RosterPlayer> getAvailableByPosition(PlayerPosition? position) {
    ...
    players.sort((a, b) => (b.projectedPoints ?? 0).compareTo(a.projectedPoints ?? 0));
    return players;
  }
  ...
  List<FreeAgentTransaction> get transactions => 
      List.unmodifiable(_transactions)..sort((a, b) => b.transactionAt.compareTo(a.transactionAt));
    ...
    try {
      ...
      for (int daysAhead = 0; daysAhead <= 3; daysAhead++) {
        ...
        for (final fixture in fixtures) {
          ...
          final isPlayerTeam = homeTeamId == player.teamId || 
          ...
          if (isPlayerTeam) {
            ...
            if (fixtureTimestamp != null) {
              final fixtureTime = DateTime.fromMillisecondsSinceEpoch(fixtureTimestamp * 1000);
              if (now.isAfter(fixtureTime)) {
                debugPrint('FreeAgency: Player ${player.name} is locked - fixture already started');
                return true;
              }
            }
          }
  ...
  Future<FreeAgentTransaction?> swapPlayers({
    ...
    debugPrint('FreeAgency: Swapped ${dropPlayer.displayName} for ${addPlayer.displayName}');
    return transaction;
  }
